...
OUTPUT_FILE = "project_structure45.md"
...
export default function Topbar() {
  ...
  // 🌟 مراجع (Refs) لإغلاق النوافذ عند الضغط خارجها
  ...
  // 🌟 بيانات وهمية للإشعارات (يمكنك لاحقاً ربطها بقاعدة البيانات)
  const [notifications, setNotifications] = useState([
    { id: 1, type: 'warning', title: 'تنبيه مخزون', desc: 'حليب صومام قارب على النفاذ (بقي 2 علبة)', time: 'منذ 10 دقائق', read: false },
    { id: 2, type: 'info', title: 'ديون موردين', desc: 'يجب تسديد دفعة شركة الوردة اليوم', time: 'منذ ساعتين', read: false },
    { id: 3, type: 'success', title: 'إغلاق وردية', desc: 'تم إغلاق وردية الكاشير (محمد) بنجاح', time: 'أمس', read: true }
  ]);
  ...
  // 🌟 روابط البحث الشمولي السريع (Command Palette)
  const quickLinks = [
  ...
  return (
    <header className="h-16 bg-slate-950 border-b border-slate-800 flex items-center justify-between px-6 sticky top-0 z-50">
      ...
      {/* 🔴 القسم الأيسر: البحث الشمولي للمدير */}
      ...
      {/* القسم الأيمن: البيانات الشخصية واللغة والإشعارات */}
      <div className="flex items-center gap-3 md:gap-4 text-slate-400">
        ...
        {!isCashier && (
          <div className="relative" ref={notifRef}>
            ...
            {isNotifOpen && (
              <div className="absolute top-12 end-0 w-80 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-top-2 z-50">
                ...
                <div className="p-2 border-t border-slate-800 bg-slate-950/50 text-center">
                  <button className="text-xs font-bold text-slate-400 hover:text-white transition-colors">عرض كل الإشعارات</button>
                </div>
              </div>
            )}
          </div>
