...
OUTPUT_FILE = "project_structure26.md"
...
function closeBusinessDay(adminName) {
  try {
    ...
    if (!summaryRes.success || summaryRes.data.shifts.length === 0) {
      return { success: false, message: 'no_shifts_to_close' };
    }
    const totals = summaryRes.data.grandTotals;

    // 1. إدخال اليومية الجديدة والحصول على الـ ID الخاص بها
    ...
    const closureId = info.lastInsertRowid; // 🔴 رقم اليومية

    // 2. تحديث الورديات وربطها برقم هذه اليومية
...
function getArchivedZReport(closureId) {
  try {
    ...
    // جلب الورديات التابعة لهذه اليومية فقط
    const shifts = db.prepare("SELECT * FROM shifts WHERE daily_closure_id = ?").all(closureId);
    
    const detailedShifts = shifts.map(shift => {
      let expensesRow, paymentsRow, advancesRow;
      // الحد الأقصى للوقت هو وقت إغلاق الوردية (لكي لا تُحسب مصاريف اليوم التالي بالخطأ)
...
module.exports = {
  ...
  getAllShiftsSummary, closeBusinessDay, getDailyClosures, getArchivedZReport // 🔴 تم تصدير الدالة الجديدة
};
```
...
const { 
  ...
  getUsers, addUser, deleteUser, updateEmployee, deleteEmployee ,logAudit , getAuditLogs, backupDatabase,
   generateExcelBackup, updateSupplier, deleteSupplier, updateAdvance, deleteAdvance, getAllShiftsSummary , getDailyClosures, getArchivedZReport
        ...
        <>
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-sm text-slate-500">{t('zreport.archive_desc')}</p>
            </div>
...
export default function AuditLogs() {
  ...
  const renderAuditDetails = (log) => {
    try {
      ...
      return t(`audit.details.${log.action}`, parsedDetails) || JSON.stringify(parsedDetails);
    } catch (e) {
      return log.details;
    }
  };
            ...
            <select value={filterAction} onChange={(e) => setFilterAction(e.target.value)} className="bg-transparent text-white focus:outline-none py-2.5">
              {uniqueActions.map(action => (
                <option key={action} value={action}>
                  {action === 'all' ? t('common.all', 'الكل') : t(`audit.actions.${action}`, { defaultValue: action })}
                </option>
              ))}
            </select>
          </div>
...
export default function Payroll() {
  ...
  const handleSaveAdvance = async (e) => {
    ...
    if (success) {
      ...
      fetchAdvances();
      showToast('success', editingAdvance ? t('common.success') : t('common.success'));
    } else {
      showToast('error', t('common.error'));
    }
  };
  ...
  const executeDeleteAdvance = async () => {
    ...
    if (window.api && window.api.deleteAdvance) {
      ...
      if (res?.success) {
        ...
        const translatedError = res.error ? t(`payroll.errors.${res.error}`, {defaultValue: res.error}) : t('common.error');
...
// 🔴 دالة توليد ملف Word الاحترافي (جدول موحد مفصل + دعم اللغتين + تنسيق صارم لسجل الحضور)
  const exportSalariesToWord = (salariesList, isSingle = false) => {
    ...
    // 🔴 الحالة الأولى: طباعة كشف مفصل لموظف واحد
    if (isSingle) {
    ...
    // 🔴 الحالة الثانية: جدول سجل الرواتب الشامل
    else {
      html += `
        <table>
          <thead>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 p-6 font-sans flex flex-col gap-6 text-start relative">
      ...
      {activeTab === 'calculator' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-lg h-fit">
            <h3 className="text-lg font-bold text-white mb-6 border-b border-slate-800 pb-4">{t('payroll.tabs.calculator')}</h3>
            <form onSubmit={handleCalculate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-400 mb-2">{t('agenda.modal.dateLabel')} (من)</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-400 mb-2">{t('agenda.modal.dateLabel')} (إلى)</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-2">{t('payroll.hourlyRate')} ({t('currency')})</label>
              ...
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg mt-4 transition-colors flex items-center justify-center gap-2">
                <Calculator size={18}/> حساب الراتب
              </button>
            </form>
          </div>
          ...
          <div className="lg:col-span-2">
            {!payrollResult ? (
              <div className="h-full bg-slate-900/50 border border-slate-800 border-dashed rounded-xl flex flex-col items-center justify-center text-slate-500 p-12 min-h-[400px]">
                ...
                <p>أدخل بيانات الموظف واضغط على "حساب الراتب" لظهور النتيجة</p>
              </div>
            ) : (
              <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95">
                <div className="bg-slate-800 p-6 flex justify-between items-center border-b border-slate-700">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">{t('payroll.period')} <bdi dir="ltr">{startDate}</bdi> {t('common.to')} <bdi dir="ltr">{endDate}</bdi></p>
                  </div>
                  <button onClick={handleExportSingleToWord} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2" title="استخراج كملف Word">
                    <Download size={18} /> استخراج Word
                  </button>
                </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-slate-950 rounded-xl p-5 border border-slate-800 flex flex-col items-center text-center">
                    ...
                    <p className="text-slate-400 text-sm mb-1">{t('payroll.totalHours')}</p>
                  ...
                  <div className="bg-slate-950 rounded-xl p-5 border border-slate-800 flex flex-col items-center text-center">
                    ...
                    <p className="text-slate-400 text-sm mb-1">{t('payroll.grossSalary')}</p>
                    <p className="text-3xl font-bold text-emerald-400"><bdi>{formatMoney(payrollResult.grossSalary)}</bdi> <span className="text-sm">{t('currency')}</span></p>
                  </div>
                  <div className="bg-red-950/30 rounded-xl p-5 border border-red-900/50 flex flex-col items-center text-center">
                    ...
                    <p className="text-red-400/80 text-sm mb-1">{t('payroll.deductions')}</p>
                    <p className="text-3xl font-bold text-red-400"><bdi dir="ltr">-{formatMoney(payrollResult.totalAdvances)}</bdi> <span className="text-sm">{t('currency')}</span></p>
                  </div>
                </div>

                <div className="bg-slate-950 p-6 border-t border-slate-800 flex justify-between items-center">
                  <div>
                    <p className="text-slate-400 mb-1">{t('payroll.netSalary')}</p>
                    <p className={`text-4xl font-black ${payrollResult.netSalary >= 0 ? 'text-white' : 'text-red-500'}`}><bdi>{formatMoney(payrollResult.netSalary)}</bdi> <span className="text-xl text-slate-500">{t('currency')}</span></p>
                  </div>
                  <button onClick={handlePaySalaryClick} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 px-8 rounded-xl flex items-center gap-2 transition-colors shadow-lg">
                    <CheckCircle size={20} /> دفع الراتب وحفظ
                  </button>
                </div>
              </div>
            )}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
          <div className="p-4 border-b border-slate-800 bg-slate-950/30 flex justify-between items-center">
            ...
            <button onClick={() => { setEditingAdvance(null); setIsAdvanceModalOpen(true); }} className="flex items-center gap-2 bg-red-600 text-white px-4 py-2 rounded-md font-medium hover:bg-red-700 transition-colors">
              <Plus size={18} /><span>إضافة سلفة</span>
            </button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-start border-collapse" dir={i18n.dir()}>
              <thead>
                <tr className="border-b border-slate-800 bg-slate-950/80">
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-start">التاريخ</th>
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-start">الموظف</th>
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-start">المبلغ</th>
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-center">المصدر (الصندوق)</th>
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-center">الحالة</th>
                  {isSuperAdmin && <th className="px-6 py-4 text-sm font-medium text-slate-400 text-center">الإجراءات</th>}
                </tr>
              </thead>
              <tbody>
                {advances.map(adv => (
                  <tr key={adv.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    ...
                    <td className="px-6 py-4 font-bold text-red-400"><bdi>{formatMoney(adv.amount)}</bdi> {t('currency')}</td>
                    <td className="px-6 py-4 text-center"><span className="px-2 py-1 rounded bg-slate-800 text-slate-300 text-xs border border-slate-700">{adv.caisse_source}</span></td>
                    <td className="px-6 py-4 text-center">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${adv.status === 'pending' ? 'bg-amber-950 text-amber-400 border-amber-900' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
                        {adv.status === 'pending' ? 'غير مسددة' : 'تم الخصم'}
                      </span>
                    </td>
                    {isSuperAdmin && (
                      <td className="px-6 py-4 text-center">
                ...
                {advances.length === 0 && <tr><td colSpan={isSuperAdmin ? "6" : "5"} className="px-6 py-12 text-center text-slate-500">{t('common.noResults')}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
          <div className="p-4 border-b border-slate-800 bg-slate-950/30 flex justify-between items-center">
            ...
            <button onClick={() => exportSalariesToWord(salaries)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
              <Download size={18} /> استخراج سجل الرواتب (Word)
            </button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-start border-collapse" dir={i18n.dir()}>
              <thead>
                <tr className="border-b border-slate-800 bg-slate-950/80">
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-start">الموظف</th>
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-start">الفترة</th>
                  ...
                  <th className="px-6 py-4 text-sm font-medium text-slate-400 text-start">{t('payroll.netSalary')}</th>
                </tr>
              </thead>
              <tbody>
                {salaries.map((s, idx) => {
                  ...
                  return (
                    <tr key={idx} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                      <td className="px-6 py-4 text-start">
                        ...
                        <div className="text-xs text-slate-500 mt-1">تاريخ الدفع: <bdi dir="ltr">{s.payment_date}</bdi></div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-start font-bold">
                        <span className={s.net_salary >= 0 ? 'text-emerald-400' : 'text-red-400'}><bdi>{formatMoney(s.net_salary)}</bdi> {t('currency')}</span>
                      </td>
                    </tr>
                  )
                })}
                {salaries.length === 0 && <tr><td colSpan="6" className="px-6 py-12 text-center text-slate-500">{t('common.noResults')}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      <Modal isOpen={isAdvanceModalOpen} onClose={() => { setIsAdvanceModalOpen(false); setEditingAdvance(null); }} title={editingAdvance ? 'تعديل السلفة' : 'تسجيل سلفة موظف'}>
        <form onSubmit={handleSaveAdvance} className="space-y-4 text-start" dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-1">المبلغ ({t('currency')})</label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-1">مصدر الأموال</label>
            <select required value={advanceData.caisseSource} onChange={e => setAdvanceData({...advanceData, caisseSource: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-blue-500">
              ...
              <option value="admin">صندوق المدير (الرئيسي)</option>
              {employees.filter(e => e.role === 'cashier').map(emp => <option key={emp.id} value={emp.name}>صندوق الكاشير: {emp.name}</option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">التاريخ</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">ملاحظة ({t('common.optional')})</label>
              <input type="text" value={advanceData.note} onChange={e => setAdvanceData({...advanceData, note: e.target.value})} placeholder="سبب السلفة..." className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-blue-500" />
            </div>
          </div>
          <div className="pt-4 flex justify-end gap-3 mt-4">
            <button type="button" onClick={() => { setIsAdvanceModalOpen(false); setEditingAdvance(null); }} className="px-4 py-2 rounded-lg font-medium text-slate-300 hover:bg-slate-800 transition-colors">{t('common.cancel')}</button>
            <button type="submit" className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">حفظ السلفة</button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmAlert 
        ...
        message={confirmModalData?.type === 'rollover' ? "الراتب الصافي بالسالب. سيتم ترحيل الديون المتبقية كسلفة جديدة للشهر القادم. هل تود الاستمرار؟" : `سيتم صرف مبلغ ${payrollResult?.netSalary} د.ج من صندوق الإدارة. هل تود تأكيد الدفع؟`}
...
```json
{
  ...
  "common": {
    ...
    "serverOnlyFeature": "هذه الميزة تعمل فقط على السيرفر الرئيسي لحماية البيانات.",
    "locked": "مقفلة",
    "cancel": "إلغاء",
    "confirm": "تأكيد",
    "optional": "اختياري",
    "networkError": "خطأ في الاتصال بالشبكة المحلية",
    "back": "رجوع",
    "close": "إغلاق الواجهة",
  ...
  "login": {
    ...
    "serverError": "فشل الاتصال بالسيرفر. يرجى المحاولة لاحقاً."
  },
  "sidebar": {
  ...
  "dashboard": {
    ...
    "kpi": {
      ...
      "expenses": "إجمالي المصاريف والسلفيات"
    },
    "charts": {
    ...
    "alerts": {
      ...
      "urgentBody": "لديك {{count}} مهام مستحقة تحتاج إلى مراجعة اليوم!",
  ...
  "suppliers": {
    ...
    "searchPlaceholder": "ابحث عن الموردين بالاسم أو الهاتف...",
    "table": {
      "name": "اسم المورد",
      "phone": "رقم الهاتف",
    ...
    "actions": {
      ...
      "importSuccess": "تم استيراد {{count}} موردين بنجاح.",
      "importError": "فشل استيراد ملف الإكسيل."
    },
    "details": {
      "addReceipt": "إضافة فاتورة (دين)",
      "addPayment": "تسديد دفعة للمورد",
      "schedulePayment": "جدولة دفعة",
      "receipts": "الفواتير (الديون) (+)",
      "payments": "المدفوعات (التسديدات) (-)",
      "amount": "المبلغ"
    },
    "messages": {
      "saveError": "فشل في حفظ بيانات المورد.",
      "deleteError": "فشل في حذف المورد.",
      "deleteProtected": "لا يمكن حذف مورد لديه سجل فواتير أو مدفوعات."
    },
    "modal": {
      "nameLabel": "الاسم",
  ...
  "hr": {
    ...
    "subtitle": "إدارة الحضور والانصراف وسجلات العمال",
    "todaysAttendance": "حضور اليوم",
    "attendanceLog": "سجل حركة الموظفين لليوم",
    "tabs": {
      "attendance": "تسجيل الحضور",
      "employees": "إدارة العمال"
    },
    "scanner": {
      "title": "تسجيل الدخول / الخروج",
      "placeholder": "أدخل رمز PIN الخاص بك أو امسح الباركود...",
      "submit": "تسجيل الحركة"
    },
    "scannerHint": "القارئ يعمل كلوحة مفاتيح. ضع المؤشر في الحقل وامسح.",
    "kpi": {
      "present": "حاضر اليوم",
      "absent": "غائب",
      "late": "متأخر"
    ...
    "table": {
      ...
      "status": "الحالة",
      "loading": "جاري التحميل...",
      "emptyRecord": "لا توجد حركات تسجيل دخول حتى الآن اليوم."
    },
    "status": {
      "present": "حاضر (في الدوام)",
      "absent": "غائب",
      "late": "متأخر",
    ...
    "employees": {
      ...
      "softDeleted": "تم تعطيل الحساب لحماية سجلاته المالية السابقة."
    },
    "dialog": {
      "title": "إضافة موظف جديد",
      "editTitle": "تعديل بيانات موظف",
    ...
    "messages": {
      ...
      "checkedIn": "تم تسجيل دخول {{name}} على الساعة {{time}}",
      "checkedOut": "تم تسجيل خروج {{name}} على الساعة {{time}}",
      "shiftCompleted": "الموظف {{name}} أتم دوامه بالفعل.",
      "invalidPin": "رمز PIN غير صحيح أو الباركود غير معروف!",
  ...
  "expenses": {
    ...
    "saveChanges": "حفظ التعديلات",
    "deleteConfirm": "هل أنت متأكد من حذف هذا المصروف؟ سيتم استرجاع المبلغ لصندوق المتجر.",
    ...
    "categories": {
      "utilities": "فواتير وخدمات (كهرباء، ماء...)",
      "maintenance": "صيانة وإصلاح",
      "supplies": "مستلزمات المتجر (أكياس، تنظيف...)",
      "advance": "سلفة عامل",
  ...
  "payroll": {
    ...
    "calculator": "حساب راتب موظف",
    "selectEmployee": "اختر الموظف",
    "startDate": "تاريخ بداية الفترة",
    "endDate": "تاريخ نهاية الفترة",
    "hourlyRate": "أجرة الساعة الواحدة",
    "calculateBtn": "احسب الراتب",
    ...
    "negativeSalaryError": "تنبيه: سلفيات الموظف أكبر من راتبه الحالي!",
    "rolloverBtn": "ترحيل الدين للشهر القادم",
    "payBtn": "دفع الراتب وأرشفته",
    ...
    "statusPaid": "تم الخصم",
    "lockedAdvance": "لا يمكن التعديل: تم خصمها من راتب مدفوع ومقفل",
    "printReport": "طباعة تقرير الرواتب",
    "noSalariesToPrint": "لا توجد سجلات رواتب لطباعتها.",
    "period": "فترة العمل",
    "selectCaisse": "-- اختر صندوق الدفع --",
    "employeeEditNotice": "لا يمكن تغيير اسم الموظف عند التعديل، لسلامة الحسابات.",
    "rolloverNote": "ترحيل ديون سلفيات من {{start}} إلى {{end}}",
    "expenseNote": "راتب الموظف ({{name}} من {{start}} إلى {{end}})",
    "rolloverConfirm": "هذا الموظف مدين للمتجر. هل تريد أرشفة الراتب وترحيل ديونه كـ (سلفة جديدة) للشهر القادم؟",
    "standardConfirm": "هل أنت متأكد من دفع هذا الراتب وأرشفته؟ سيتم سحب المبلغ من صندوق المدير.",
    "deleteAdvanceTitle": "حذف السلفة",
    "deleteAdvanceMsg": "هل أنت متأكد من إلغاء هذه السلفة؟ سيتم استرجاع قيمتها مباشرة إلى صندوق المصاريف الخاص بها.",
    "errors": {
      "advanceNotFound": "لم يتم العثور على السلفة في قاعدة البيانات.",
      "cannotDeletePaid": "عذراً، لا يمكن حذف سلفة تم دفعها مسبقاً ومخصومة من الراتب."
    },
  ...
  "agenda": {
    ...
    "subtitle": "إدارة المواعيد، التوصيلات، والمدفوعات المجدولة",
    "addTask": "إضافة مهمة",
    "filters": {
      "all": "كل المهام",
  ...
  "settings": {
    ...
    "subtitle": "إدارة المستخدمين، الصلاحيات، وقاعدة البيانات",
    "newUser": "إنشاء حساب مستخدم جديد",
    "errorFillFields": "يرجى تعبئة جميع الحقول.",
    "addError": "فشل في إنشاء الحساب.",
    "deleteAlert": "لا يمكنك حذف حساب المدير العام أو حسابك الشخصي.",
    "deleteError": "فشل في حذف المستخدم.",
    "accessDenied": "تم رفض الوصول. هذه الصفحة مخصصة للمدير العام فقط.",
    ...
    "deleteConfirm": "هل أنت متأكد من حذف حساب المستخدم:\n{{name}}؟",
    "username": "اسم المستخدم (Username)",
    "password": "كلمة المرور / PIN",
    "role": "الصلاحية (المنصب)",
    "addAccountBtn": "إنشاء الحساب",
    "modal": {
      "title": "إعدادات المتجر",
      "storeNameLabel": "اسم المتجر (يظهر في التذاكر والتقارير)",
      "storeNamePlaceholder": "مثال: سوبرماركت السعادة",
  ...
  "database": {
    ...
    "backupDesc": "قم بحفظ نسخة آمنة من قاعدة بياناتك وتقارير الإكسيل على حاسوبك.",
    "restore": "استرجاع نسخة احتياطية",
    "restoreDesc": "استعادة النظام بالكامل من ملف نسخة احتياطية محفوظ مسبقاً.",
    "messages": {
      "backupSuccess": "تم حفظ النسخة الاحتياطية وتقارير الإكسيل بنجاح!",
      "restoreSuccess": "تم استرجاع النسخة الاحتياطية بنجاح. سيتم إعادة تشغيل النظام.",
      "restoreConfirm": "تحذير: استرجاع نسخة احتياطية سيقوم بمسح كافة البيانات الحالية وتعويضها ببيانات النسخة. هل أنت متأكد؟",
      "error": "حدث خطأ أثناء معالجة قاعدة البيانات."
    }
  },
  "pos": {
    "title": "نقطة البيع (POS)",
    "scanPlaceholder": "قم بمسح الباركود أو ابحث عن منتج...",
    "cart": "سلة المشتريات",
  ...
  "eod": {
    "title": "إغلاق الوردية الصندوق",
    "open_shift_title": "فتح وردية عمل جديدة",
    "open_shift_desc": "مرحباً بك،",
    "opening_balance": "رصيد الصندوق عند الفتح (النقدية الحالية)",
    "open_shift_btn": "بدء الوردية",
    "active_shift": "الوردية النشطة حالياً",
    "total_deducted": "المصاريف المسحوبة من الدرج",
    "advances": "سلفيات العمال",
    "supplier_payments": "تسديدات الموردين",
    "actual_cash": "المبلغ النقدي المحسوب فعلياً في الدرج",
    "actualCashHint": "قم بعد النقود الموجودة في الدرج حالياً واكتب المجموع هنا.",
    "notes": "ملاحظات الوردية (اختياري)",
    "notesPlaceholder": "سجل أي فوارق، أخطاء، أو ملاحظات للمدير...",
    "today_sales": "مبيعات الوردية المحسوبة",
    "save_btn": "إغلاق الوردية وقفل الصندوق",
    "confirmClose": "هل أنت متأكد من إغلاق هذه الوردية؟ سيتم قفل الحسابات ولا يمكن التراجع عن هذا الإجراء.",
    "masterDashboardTitle": "لوحة المراقبة الشاملة للورديات",
    "masterDashboardDesc": "مراقبة حية لصناديق الكاشير وإجمالي المبيعات",
    "grandTotalOpening": "إجمالي الرصيد الافتتاحي للأدراج",
    "grandTotalActual": "إجمالي النقود الفعلية المستلمة",
    "grandTotalSales": "صافي أرباح الورديات المغلقة",
    "allShifts": "سجل الورديات",
    "cashierName": "اسم الكاشير",
    "timing": "التوقيت (فتح/إغلاق)",
    "statusOpen": "مفتوحة 🟢",
    "statusClosed": "مغلقة 🔴",
    "x_report": "وصل إغلاق وردية (X-Report)",
    "store_name": "متجري (MY STORE)",
    "time_out": "وقت الإغلاق",
    "receipt_footer": "تم إغلاق الوردية وتسليم العهدة.",
    "print_receipt": "طباعة الوصل",
    "close_day_btn": "إغلاق اليومية (Z-Report)",
    "close_day_confirm": "هل أنت متأكد من إغلاق اليومية؟ سيتم ترحيل الأموال للخزينة وتصفير الشاشة لليوم التالي.",
    "no_shifts_to_close": "لا يوجد أي ورديات مغلقة لترحيلها."
  },
  "print": {
    ...
    "period": "فترة العمل",
    "rate": "الأجر / ساعة",
    "receiptTicket": "وصل استلام",
    "thankYou": "شكراً لتعاملكم معنا"
  },
  "backendErrors": {
    "userExists": "اسم المستخدم أو رمز PIN مستخدم بالفعل.",
    "employeeExists": "اسم الموظف أو رمز PIN مستخدم بالفعل من قبل شخص آخر!",
    "invalidCredentials": "اسم المستخدم أو كلمة المرور غير صحيحة.",
    "invalidPinOrInactive": "رمز PIN غير صحيح أو حساب معطل.",
    "shiftAlreadyOpen": "لديك وردية مفتوحة بالفعل. لا يمكنك فتح وردية أخرى.",
    "has_open_shifts": "عذراً! لا يمكن إغلاق اليومية وهناك كاشير لم يغلق صندوقه بعد.",
    "alreadyCompletedShift": "الموظف {{name}} أتم تسجيل الحضور والانصراف اليوم."
  },
  "audit": {
    "actions": {
  ...
  "zreport": {
    "title": "التقرير المالي اليومي الشامل (Z-Report)",
    "date": "تاريخ التقرير:",
    "closed_by": "أُغلق بواسطة:",
    "summary": "الملخص المالي الإجمالي",
    "opening": "الرصيد الافتتاحي",
    "net_sales": "صافي المبيعات",
    "actual_cash": "النقدية الفعلية بالخزينة",
    "shifts_details": "تفاصيل ورديات الكاشير",
    "cashier": "الكاشير",
    "time_in": "الدخول",
    "time_out": "الخروج",
    "deductions": "المسحوبات",
    "sales": "المبيعات",
    "actual_drawer": "الفعلي بالدرج",
    "manager_sig": "توقيع المدير",
    "company_seal": "ختم المؤسسة",
    "not_closed": "لم يغلق",
    "archive_title": "أرشيف إغلاق اليوميات"
  }
}
```
...
```json
{
  ...
  "common": {
    ...
    "serverOnlyFeature": "This feature only works on the main server.",
    "locked": "Locked",
    "cancel": "Cancel",
    "confirm": "Confirm",
    "optional": "Optional",
  ...
  "dashboard": {
    ...
    "subtitle": "System overview & financial metrics",
    "quickAction": "Quick Action",
    "quickActionExpense": "New Expense",
    "kpi": {
      "totalDebts": "Total Debts (Suppliers)",
      "dueThisWeek": "Due This Week",
      "activeEmployees": "Active Employees",
    ...
    "lists": {
      "urgentAlerts": "Urgent Agenda Alerts",
      "recentAudit": "Recent Audit Logs",
      "noAuditLogs": "No recent activity found."
    },
    "alerts": {
      "systemTitle": "System Alerts ⚠️",
      "urgentBody": "You have {{count}} urgent pending tasks today!",
  ...
  "suppliers": {
    ...
    "actions": {
      ...
      "importError": "Failed to import excel file."
    },
    "details": {
    ...
    "messages": {
      ...
      "deleteProtected": "Cannot delete supplier with transaction history."
    },
    "modal": {
      "nameLabel": "Name",
  ...
  "hr": {
    ...
    "scannerHint": "Scanner acts as keyboard. Focus field and scan.",
    "kpi": {
      "present": "Present Today",
      "absent": "Absent",
      "late": "Late"
    ...
    "table": {
      "name": "Employee Name",
      "nameWithRole": "Employee",
      "role": "Position",
      "timeIn": "Time In",
      "timeOut": "Time Out",
      "status": "Status",
      "loading": "Loading...",
      "emptyRecord": "No attendance records found for today."
    },
    "status": {
      "present": "Present",
      "absent": "Absent",
    ...
    "employees": {
      ...
      "deleteConfirmMsg": "Are you sure you want to delete the account for:\\n{{name}}?",
      "softDeleted": "Account disabled to protect financial records."
    },
    "dialog": {
      "title": "Add New Employee",
      "editTitle": "Edit Employee",
      "desc": "Enter employee details and secure PIN.",
      "name": "Full Name",
    ...
    "messages": {
      ...
      "checkedIn": "{{name}} Checked IN at {{time}}",
      "checkedOut": "{{name}} Checked OUT at {{time}}",
      "shiftCompleted": "{{name}} has already completed their shift.",
      "invalidPin": "Invalid PIN or Barcode not recognized!",
      "error": "An unexpected error occurred."
    }
  },
  "expenses": {
    "title": "Expenses & Operations",
    "subtitle": "Track daily store expenses and money flow",
    "addExpense": "Add Expense",
    "editExpense": "Edit Expense",
    "saveChanges": "Save Changes",
    "deleteConfirm": "Are you sure you want to delete this transaction? Money will be returned to the register.",
    "allCaisses": "All Registers",
    "adminCaisse": "Main Admin Register",
    "cashierCaisse": "Cashier: {{name}}",
    "caisseSourceLabel": "Fund Source (Register)",
    "myAdvance": "My Personal Advance",
    "dateLabel": "Date",
    "cashierNotice": "You are viewing only the transactions made from your own register ({{name}}).",
    "kpi": {
  ...
  "payroll": {
    ...
    "subtitle": "Calculate monthly salaries and manage advances",
    "tabs": {
    ...
    "negativeSalaryError": "Warning: Deductions exceed gross salary!",
    ...
    "date": "Date",
    "caisse": "Register Source",
    "amount": "Amount",
    "note": "Note/Description",
    "statusPending": "Pending (Not Deducted)",
    "statusPaid": "Deducted",
    "lockedAdvance": "Cannot edit: Already deducted from a paid salary",
    "printReport": "Print Report",
    "noSalariesToPrint": "No salary records available to print.",
    "period": "Period",
    "selectCaisse": "-- Select Source --",
    "employeeEditNotice": "Employee name cannot be changed during edit for accounting safety.",
    "rolloverNote": "Rolled over debt from {{start}} to {{end}}",
    "expenseNote": "Salary for {{name}} ({{start}} to {{end}})",
    "rolloverConfirm": "Employee owes the store. Do you want to rollover the debt to the next month as an advance?",
    "standardConfirm": "Are you sure you want to mark this salary as PAID? The amount will be deducted from the main register.",
    "deleteAdvanceTitle": "Delete Advance",
    "deleteAdvanceMsg": "Are you sure you want to delete this advance? The amount will be returned to the register.",
    "errors": {
      "advanceNotFound": "Advance record not found in the database.",
      "cannotDeletePaid": "Cannot delete an advance that has already been paid/deducted."
    },
    "accountantSig": "Recipient's Signature",
    "comprehensiveReport": "Detailed Payroll & Attendance Report",
  ...
  "agenda": {
    "title": "Agenda & Tasks",
    "subtitle": "Manage schedules, deliveries, and payments",
    "addTask": "New Task",
    "filters": {
      "all": "All Tasks",
      "pending": "Pending",
  ...
  "settings": {
    ...
    "subtitle": "Manage users, permissions, and database",
    "newUser": "Create New User",
    "errorFillFields": "Please fill all fields.",
    "addError": "Failed to create user.",
    "deleteAlert": "You cannot delete the main admin or your own account.",
    ...
    "modal": {
      ...
      "storeNameLabel": "Store Name (Receipts & Reports)",
      "storeNamePlaceholder": "e.g. ULTRA JOY",
  ...
  "database": {
    ...
    "backupDesc": "Save a secure copy of your database and excel reports to your computer.",
    "restore": "Restore Database",
    "restoreDesc": "Restore the system from a previously saved backup file.",
    "messages": {
      "backupSuccess": "Database backup saved successfully!",
      "restoreSuccess": "Database restored successfully. System will restart.",
      "restoreConfirm": "Warning: Restoring a database will overwrite current data. Are you sure?",
      "error": "Database operation failed."
    }
  },
  "pos": {
    "title": "Point of Sale",
    "scanPlaceholder": "Scan Barcode or Search Product...",
    "cart": "Shopping Cart",
  ...
  "eod": {
    ...
    "open_shift_title": "Open Your Shift",
    "open_shift_desc": "Welcome,",
    "opening_balance": "Opening Cash in Drawer",
    "open_shift_btn": "Start Shift",
    "active_shift": "Active Shift",
    "total_deducted": "Total Outflow (Expenses)",
    "advances": "Advances",
    "supplier_payments": "Supplier Payments",
    "actual_cash": "Actual Cash Counted",
    "actualCashHint": "Count the physical cash currently inside the drawer.",
    "notes": "Shift Notes (Optional)",
    "notesPlaceholder": "Any discrepancies or notes for the manager...",
    "today_sales": "Calculated Shift Sales",
    "save_btn": "Close Shift & Lock",
    "confirmClose": "Are you sure you want to close this shift? This action cannot be undone.",
    "masterDashboardTitle": "Master Shifts Dashboard",
    "masterDashboardDesc": "Live monitoring of all cash registers and total sales",
    "grandTotalOpening": "Total Opening Balance",
    "grandTotalActual": "Total Actual Cash Received",
    "grandTotalSales": "Net Profit from Closed Shifts",
    "allShifts": "Shifts Log",
    "cashierName": "Cashier Name",
    "timing": "Timing (Start/End)",
    "statusOpen": "Open 🟢",
    "statusClosed": "Closed 🔴",
    "x_report": "Shift Closure (X-Report)",
    "store_name": "MY STORE",
    "time_out": "Time Out",
    "receipt_footer": "Shift closed and cash handed over.",
    "print_receipt": "Print Receipt",
    "close_day_btn": "Close Business Day (Z-Report)",
    "close_day_confirm": "Are you sure you want to close the business day? All shifts will be archived.",
    "no_shifts_to_close": "No closed shifts to archive."
  },
  "print": {
    "printBtn": "Print",
  ...
  "backendErrors": {
    "userExists": "Username or PIN is already in use.",
    "employeeExists": "Employee name or PIN is already in use!",
    "invalidCredentials": "Invalid username or password.",
    "invalidPinOrInactive": "Invalid PIN or account is disabled.",
    "shiftAlreadyOpen": "You already have an open shift.",
    "has_open_shifts": "Cannot close day! There are still open shifts.",
    "alreadyCompletedShift": "{{name}} has already completed today's shift."
  },
  "audit": {
    "actions": {
    ...
    "details": {
      "DELETE_EXPENSE": "Deleted an expense ({{desc}}) with amount {{amount}}"
    },
    "subtitle": "Monitor and track all system and user activity"
  },
  "zreport": {
    "title": "Comprehensive Daily Financial Report (Z-Report)",
    "date": "Report Date:",
    "closed_by": "Closed By:",
    "summary": "Overall Financial Summary",
    "opening": "Opening Balance",
    "net_sales": "Net Sales",
    "actual_cash": "Actual Cash in Safe",
    "shifts_details": "Cashier Shifts Details",
    "cashier": "Cashier",
    "time_in": "Time In",
    "time_out": "Time Out",
    "deductions": "Deductions",
    "sales": "Sales",
    "actual_drawer": "Actual in Drawer",
    "manager_sig": "Manager Signature",
    "company_seal": "Company Seal",
    "not_closed": "Not Closed",
    "archive_title": "Daily Closures Archive"
  }
}
```
...
```json
{
  ...
  "common": {
    ...
    "refresh": "Actualiser",
    "superAdmin": "Super Admin",
    "systemOwner": "Propriétaire du système",
    "edit": "Modifier",
    "delete": "Supprimer",
    ...
    "serverOnlyFeature": "Cette fonctionnalité n'est disponible que sur le serveur principal, pour protéger les données.",
    "locked": "Verrouillé",
    "cancel": "Annuler",
    "confirm": "Confirmer",
    "optional": "Facultatif",
    "networkError": "Erreur de connexion au réseau",
    "back": "Retour",
    "close": "Fermer",
  ...
  "login": {
    ...
    "serverError": "Échec de la connexion au serveur. Veuillez réessayer."
  },
  "sidebar": {
  ...
  "dashboard": {
    ...
    "subtitle": "Aperçu du système et indicateurs financiers",
    "quickAction": "Action rapide",
    "quickActionExpense": "Nouvelle dépense",
    "kpi": {
      "totalDebts": "Dettes totales (Fournisseurs)",
      "dueThisWeek": "À échéance cette semaine",
      "activeEmployees": "Employés actifs",
      "expenses": "Dépenses et avances"
    },
    "charts": {
      "topCreditors": "Top 5 des créanciers",
      "expensesDist": "Répartition des dépenses"
    },
    "lists": {
      "urgentAlerts": "Alertes urgentes de l'agenda",
      "recentAudit": "Journal d'audit récent",
      "noAuditLogs": "Aucune activité récente trouvée."
    },
    "alerts": {
      "systemTitle": "Alertes système ⚠️",
      "urgentBody": "Vous avez {{count}} tâches urgentes en attente aujourd'hui !",
  ...
  "suppliers": {
    ...
    "subtitle": "Gérer les comptes fournisseurs et les factures impayées",
    "addSupplier": "Nouveau fournisseur",
    "searchPlaceholder": "Rechercher un fournisseur par nom ou téléphone...",
    "table": {
    ...
    "actions": {
      ...
      "importError": "Échec de l'importation du fichier Excel."
    },
    "details": {
      "addReceipt": "Ajouter une facture (dette)",
      "addPayment": "Régler un paiement au fournisseur",
      "schedulePayment": "Planifier",
      "receipts": "Factures (dettes) (+)",
    ...
    "messages": {
      ...
      "deleteProtected": "Impossible de supprimer un fournisseur ayant un historique de factures ou de paiements."
    },
    "modal": {
      "nameLabel": "Nom",
  ...
  "hr": {
    "title": "RH & Personnel",
    "subtitle": "Gérer la présence, les équipes et les dossiers des employés",
    "todaysAttendance": "Présence du jour",
    "attendanceLog": "Journal de présence du jour",
    "tabs": {
      "attendance": "Scanner de présence",
      "employees": "Gérer les employés"
    },
    "scanner": {
      "title": "Pointeuse (Entrée / Sortie)",
      "placeholder": "Scannez le code-barres ou saisissez le PIN...",
      "submit": "Enregistrer la présence"
    },
    "scannerHint": "Le scanner agit comme un clavier. Placez le curseur dans le champ et scannez.",
    "kpi": {
    ...
    "table": {
      ...
      "nameWithRole": "Employé",
      "role": "Poste",
      "timeIn": "Heure d'entrée",
      "timeOut": "Heure de sortie",
      "status": "Statut",
      "loading": "Chargement...",
      "emptyRecord": "Aucune présence enregistrée pour aujourd'hui."
    },
    "status": {
      "present": "Présent",
      "absent": "Absent",
    ...
    "employees": {
      ...
      "deleteConfirmMsg": "Êtes-vous sûr de vouloir supprimer le compte de :\\n{{name}} ?",
      "softDeleted": "Compte désactivé afin de protéger ses anciens registres financiers."
    },
    "dialog": {
      "title": "Ajouter un nouvel employé",
      "editTitle": "Modifier l'employé",
      "desc": "Saisissez les informations de l'employé et son code PIN secret.",
      "name": "Nom complet",
      "namePlaceholder": "Jean Dupont...",
      "role": "Poste",
      "rolePlaceholder": "Sélectionner un poste",
      "pin": "Code PIN secret",
    ...
    "messages": {
      ...
      "checkedIn": "{{name}} a pointé son entrée à {{time}}",
      "checkedOut": "{{name}} a pointé sa sortie à {{time}}",
      "shiftCompleted": "{{name}} a déjà terminé son service.",
      "invalidPin": "PIN invalide ou code-barres non reconnu !",
      "error": "Une erreur inattendue s'est produite lors du traitement."
    }
  },
  "expenses": {
    "title": "Dépenses et opérations",
    "subtitle": "Suivre les dépenses quotidiennes du magasin et les mouvements de trésorerie",
    "addExpense": "Ajouter une dépense",
    "editExpense": "Modifier la dépense",
    "saveChanges": "Enregistrer les modifications",
    "deleteConfirm": "Êtes-vous sûr de vouloir supprimer cette transaction ? Le montant sera restitué à la caisse.",
    "allCaisses": "Toutes les caisses",
    "adminCaisse": "Caisse principale (Admin)",
    "cashierCaisse": "Caisse du caissier : {{name}}",
    "caisseSourceLabel": "Source des fonds (Caisse)",
    "myAdvance": "Mon avance personnelle",
    "dateLabel": "Date",
    "cashierNotice": "Vous ne voyez que les transactions effectuées depuis votre propre caisse ({{name}}), toutes journées confondues.",
    "kpi": {
    ...
    "categories": {
      "utilities": "Factures et services (électricité, eau...)",
      "maintenance": "Maintenance et réparation",
      "supplies": "Fournitures du magasin (sacs, nettoyage...)",
  ...
  "payroll": {
    "title": "Salaires et avances",
    "subtitle": "Calculer les salaires mensuels et gérer les avances",
    "tabs": {
      "calculator": "Calculateur de salaire",
      "advances": "Historique des avances",
      "salaries": "Salaires payés"
    },
    "calculator": "Calculer le salaire",
    ...
    "negativeSalaryError": "Attention : les déductions dépassent le salaire brut !",
    "rolloverBtn": "Reporter la dette",
    "payBtn": "Payer et archiver",
    "previewPayslip": "Aperçu du bulletin de salaire",
    "advancesTitle": "Registre des avances",
    "addAdvance": "Accorder une avance",
    "editAdvance": "Modifier l'avance",
    "date": "Date",
    "caisse": "Caisse de paiement",
    "amount": "Montant de l'avance",
    "note": "Remarques",
    "statusPending": "En attente (non déduite)",
    "statusPaid": "Déduite",
    "lockedAdvance": "Modification impossible : déjà déduite d'un salaire payé et verrouillé",
    "printReport": "Imprimer le rapport de paie",
    "noSalariesToPrint": "Aucun enregistrement de salaire disponible à imprimer.",
    "period": "Période de travail",
    "selectCaisse": "-- Sélectionner une caisse --",
    "employeeEditNotice": "Le nom de l'employé ne peut pas être modifié lors de l'édition, pour la sécurité des comptes.",
    "rolloverNote": "Dette d'avances reportée du {{start}} au {{end}}",
    "expenseNote": "Salaire de {{name}} (du {{start}} au {{end}})",
    "rolloverConfirm": "Cet employé est débiteur envers le magasin. Voulez-vous archiver le salaire et reporter sa dette sous forme de (nouvelle avance) au mois prochain ?",
    "standardConfirm": "Êtes-vous sûr de vouloir marquer ce salaire comme PAYÉ et l'archiver ? Le montant sera prélevé sur la caisse principale.",
    "deleteAdvanceTitle": "Supprimer l'avance",
    "deleteAdvanceMsg": "Êtes-vous sûr de vouloir annuler cette avance ? Son montant sera restitué directement à la caisse dont elle a été prélevée.",
    "errors": {
      "advanceNotFound": "Avance introuvable dans la base de données.",
      "cannotDeletePaid": "Désolé, impossible de supprimer une avance déjà payée et déduite du salaire."
    },
    "accountantSig": "Signature du bénéficiaire",
    "comprehensiveReport": "Registre détaillé des salaires et des présences",
    "payslip": "Bulletin de salaire employé",
    "reportTitle": "Rapport complet des salaires et mouvements de présence",
    "totalEmployees": "Total des employés"
  },
  "agenda": {
    "title": "Agenda et tâches",
    "subtitle": "Gérer les rendez-vous, livraisons et paiements programmés",
    ...
    "rescheduleTask": "Reporter la tâche",
    "deleteConfirm": "Êtes-vous sûr de vouloir supprimer cette tâche de l'agenda ?",
    "types": {
      "delivery": "Réception de commande",
      "payment": "Règlement de dette",
      "maintenance": "Maintenance"
    },
    "modal": {
      ...
      "timeLabel": "Heure",
      "cancelBtn": "Annuler",
      "saveBtn": "Enregistrer la tâche"
    }
  },
  "settings": {
    "title": "Paramètres système",
    "subtitle": "Gérer les utilisateurs, les permissions et la base de données",
    "newUser": "Créer un nouveau compte utilisateur",
    "errorFillFields": "Veuillez remplir tous les champs.",
    "addError": "Échec de la création du compte.",
    "deleteAlert": "Vous ne pouvez pas supprimer le compte de l'administrateur principal ni votre propre compte.",
    "deleteError": "Échec de la suppression de l'utilisateur.",
    "accessDenied": "Accès refusé. Cette page est réservée à l'administrateur principal.",
    ...
    "username": "Nom d'utilisateur",
    "password": "Mot de passe / PIN",
    "role": "Rôle (Poste)",
    "addAccountBtn": "Créer le compte",
    "modal": {
      "title": "Paramètres du magasin",
      "storeNameLabel": "Nom du magasin (reçus et rapports)",
      "storeNamePlaceholder": "ex. ULTRA JOY",
  ...
  "database": {
    ...
    "backupDesc": "Enregistrez une copie sécurisée de votre base de données et des rapports Excel sur votre ordinateur.",
    "restore": "Restaurer la base de données",
    "restoreDesc": "Restaurer entièrement le système à partir d'un fichier de sauvegarde préalablement enregistré.",
    "messages": {
      "backupSuccess": "Sauvegarde et rapports Excel enregistrés avec succès !",
      "restoreSuccess": "Base de données restaurée avec succès. Le système va redémarrer.",
      "restoreConfirm": "Attention : la restauration d'une sauvegarde effacera toutes les données actuelles et les remplacera par celles de la sauvegarde. Êtes-vous sûr ?",
      "error": "Une erreur est survenue lors du traitement de la base de données."
    }
  },
  "pos": {
    "title": "Point de vente",
    "scanPlaceholder": "Scannez le code-barres ou recherchez un produit...",
    "cart": "Panier",
    "emptyCart": "Le panier est vide",
    "total": "Montant total",
    "pay": "Payer et imprimer",
    "perUnit": "par unité"
  },
  "eod": {
    "title": "Gestion des services de caisse",
    "open_shift_title": "Ouvrir un nouveau service",
    "open_shift_desc": "Bienvenue,",
    "opening_balance": "Fonds de caisse à l'ouverture (liquide actuel)",
    "open_shift_btn": "Démarrer le service",
    "active_shift": "Service actuellement actif",
    "total_deducted": "Dépenses prélevées du tiroir",
    "advances": "Avances aux employés",
    "supplier_payments": "Paiements fournisseurs",
    "actual_cash": "Montant en espèces réellement compté dans le tiroir",
    "actualCashHint": "Comptez l'argent actuellement présent dans le tiroir-caisse et indiquez le total ici.",
    "notes": "Notes de service (facultatif)",
    "notesPlaceholder": "Notez tout écart, erreur ou remarque à l'attention du responsable...",
    "today_sales": "Ventes calculées du service",
    "save_btn": "Clôturer le service et verrouiller la caisse",
    "confirmClose": "Êtes-vous sûr de vouloir clôturer ce service ? Cette action est irréversible.",
    "masterDashboardTitle": "Tableau de bord général des services",
    "masterDashboardDesc": "Surveillance en direct de toutes les caisses et des ventes totales",
    "grandTotalOpening": "Total des fonds de caisse à l'ouverture",
    "grandTotalActual": "Total des espèces réellement reçues",
    "grandTotalSales": "Bénéfice net des services clôturés",
    "allShifts": "Journal des services",
    "cashierName": "Nom du caissier",
    "timing": "Horaire (Ouverture/Fermeture)",
    "statusOpen": "Ouvert 🟢",
    "statusClosed": "Fermé 🔴",
    "x_report": "Clôture de service (Rapport X)",
    "store_name": "MON MAGASIN",
    "time_out": "Heure de clôture",
    "receipt_footer": "Service clôturé et caisse remise.",
    "print_receipt": "Imprimer le reçu",
    "close_day_btn": "Clôturer la journée (Rapport Z)",
    "close_day_confirm": "Êtes-vous sûr de vouloir clôturer la journée ? Les fonds seront transférés au coffre et l'écran sera réinitialisé pour le lendemain.",
    "no_shifts_to_close": "Aucun service clôturé à archiver."
  },
  "print": {
    ...
    "date": "Date",
    "deductions": "Déductions (avances)",
    "description": "Description / Remarque",
    "employeeName": "Nom de l'employé",
    "employeeSignature": "Signature de l'employé",
    "grossSalary": "Salaire de base",
    "hours": "Heures",
    "managerSignature": "Signature du responsable",
    "netSalary": "Net à payer",
    "paymentTicket": "Reçu de paiement",
    "payslipTitle": "Bulletin de salaire",
    "period": "Période de travail",
    "rate": "Taux / heure",
    "receiptTicket": "Reçu",
  ...
  "backendErrors": {
    "userExists": "Ce nom d'utilisateur ou ce PIN est déjà utilisé.",
    "employeeExists": "Ce nom d'employé ou ce PIN est déjà utilisé par quelqu'un d'autre !",
    "invalidCredentials": "Nom d'utilisateur ou mot de passe incorrect.",
    "invalidPinOrInactive": "PIN invalide ou compte désactivé.",
    "shiftAlreadyOpen": "Vous avez déjà un service ouvert. Vous ne pouvez pas en ouvrir un autre.",
    "has_open_shifts": "Impossible de clôturer la journée ! Un caissier n'a pas encore clôturé sa caisse.",
    "alreadyCompletedShift": "{{name}} a déjà pointé son entrée et sa sortie aujourd'hui."
  },
  "audit": {
    "actions": {
    ...
    "details": {
      "DELETE_EXPENSE": "A supprimé une dépense ({{desc}}) d'un montant de {{amount}} DA"
    },
    "subtitle": "Surveiller et suivre toutes les activités du système et des utilisateurs"
  },
  "zreport": {
    "title": "Rapport financier journalier complet (Rapport Z)",
    "date": "Date du rapport :",
    "closed_by": "Clôturé par :",
    "summary": "Résumé financier global",
    "opening": "Solde d'ouverture",
    "net_sales": "Ventes nettes",
    "actual_cash": "Espèces réelles en coffre",
    "shifts_details": "Détails des services des caissiers",
    "cashier": "Caissier",
    "time_in": "Entrée",
    "time_out": "Sortie",
    "deductions": "Prélèvements",
    "sales": "Ventes",
    "actual_drawer": "Réel dans le tiroir",
    "manager_sig": "Signature du responsable",
    "company_seal": "Cachet de l'entreprise",
    "not_closed": "Non clôturé",
    "archive_title": "Archives des clôtures journalières"
  }
}
```
