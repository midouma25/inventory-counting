...
OUTPUT_FILE = "project_structure21.md"
...
export default function Expenses() {
  ...
  // 🔴 تحديد نوع المستخدم واسم صندوقه
  const isSuperAdmin = user?.role === 'superadmin' || user?.role === 'admin';
  const myCaisseName = isSuperAdmin ? 'admin' : (user?.username || 'Cashier');
  
  // 🔴 الفلتر الافتراضي: المدير يرى (all)، والكاشير يرى (myCaisseName)
  const [selectedCaisseFilter, setSelectedCaisseFilter] = useState(isSuperAdmin ? 'all' : myCaisseName);

  const [formData, setFormData] = useState({
    description: '', category: 'utilities', amount: '', employeeId: '', supplierId: '', caisseSource: myCaisseName
  });
  
  const fetchExpensesList = async () => {
    try {
      if (window.api && window.api.getExpenses) {
        // نمرر الفلتر للسيرفر لجلب مصاريف هذا الصندوق فقط (وكل الأيام السابقة)
  ...
  const openAddModal = () => {
    ...
    setFormData({ description: '', category: 'utilities', amount: '', employeeId: '', supplierId: '', caisseSource: myCaisseName });
    setIsModalOpen(true);
  };
  ...
  // قائمة صناديق الكاشير ليختار منها السوبر أدمن
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 p-6 font-sans text-start relative">
          ...
          {isSuperAdmin && (
             <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 shadow-sm">
               ...
               >
                 ...
                 {availableCaisses.map(c => <option key={c} value={c}>{c === 'admin' ? 'صندوق المدير' : `صندوق الكاشير: ${c}`}</option>)}
               </select>
             </div>
          )}
      ...
      {/* 🔴 تنبيه صغير يظهر للكاشير ليعرف أنه يرى مصاريفه فقط */}
{/* 🔴 تنبيه صغير يظهر للكاشير ليعرف أنه يرى مصاريفه فقط (يدعم اللغتين الآن) */}
      {!isSuperAdmin && (
          ...
          <table className="w-full text-start border-collapse" dir={i18n.dir()}>
            <thead>
              <tr className="border-b border-slate-800 bg-slate-950/80">
                ...
                {isSuperAdmin && <th className="px-6 py-4 text-sm font-medium text-slate-400 text-center">المصدر (Caisse)</th>}
              ...
              {filteredExpenses.map((exp) => (
                <tr key={`${exp.source}-${exp.id}`} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                  ...
                  {isSuperAdmin && (
                     <td className="px-6 py-4 text-center">
                       <span className={`px-2 py-1 rounded-md text-xs font-medium ${exp.caisse_source === 'admin' ? 'bg-blue-950 text-blue-400 border border-blue-900' : 'bg-slate-800 text-slate-300 border border-slate-700'}`}>
                         {exp.caisse_source === 'admin' ? 'المدير' : exp.caisse_source}
                       </span>
                     </td>
                  )}
                  ...
                  <td className="px-6 py-4 text-center">
                    {exp.source === 'expense' ? (
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => setExpenseToDelete(exp.id)} className="p-2 text-red-400 hover:bg-red-900/50 rounded-lg transition-colors"><Trash2 size={18} /></button>
                      </div>
                    ) : (
                      <div className="flex items-center justify-center text-xs text-slate-500 gap-1" title={t('common.serverOnlyFeature')}>
                        <ShieldAlert size={14} /> مقفل
                      </div>
                    )}
                  </td>
                </tr>
      ...
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingExpense(null); }} title={editingExpense ? t('expenses.editExpense') : t('expenses.addExpense')}>
        <form className="space-y-4" onSubmit={handleSubmitExpense} dir={isRTL ? "rtl" : "ltr"}>
          ...
          {/* 🔴 يظهر هذا الخيار للمدير فقط لاختيار الصندوق الذي دفعت منه الأموال */}
          {isSuperAdmin && !editingExpense && (
             <div>
               <label className="block text-sm font-medium text-slate-400 mb-1 text-start">مصدر الأموال (الصندوق)</label>
               <select value={formData.caisseSource} onChange={e => setFormData({...formData, caisseSource: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-blue-500 text-start">
                 ...
                 {availableCaisses.filter(c => c !== 'admin').map(c => <option key={c} value={c}>صندوق الكاشير: {c}</option>)}
               </select>
             </div>
          )}
