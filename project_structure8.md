...
OUTPUT_FILE = "project_structure8.md"
...
const EndOfDay = () => {
  ...
  const expectedAmount = parseFloat(openingBalance || 0) - parseFloat(summary.totalOut || 0);
  const difference = parseFloat(actualAmount || 0) - expectedAmount;

  const handleSaveEOD = async () => {
    console.log("Saving...", { date, expectedAmount, actualAmount, difference, notes });
  };
    ...
    <div className="p-6 bg-gray-900 text-white min-h-screen rounded-lg">
      ...
      <div className="p-6 bg-gray-800 rounded-lg shadow-lg mb-6">
        ...
        <div className="flex justify-between items-center border-t border-gray-600 pt-4 mt-4">
          <div>
            <h3 className="text-gray-400">{t('eod.expected_cash')}</h3>
            <p className="text-2xl font-bold">{expectedAmount} {t('currency')}</p>
          </div>
          <div>
            <h3 className="text-gray-400">{t('eod.difference')}</h3>
            <p className={`text-3xl font-bold ${difference < 0 ? 'text-red-500' : difference > 0 ? 'text-green-500' : 'text-gray-300'}`}>
...
```json
{
  ...
  "eod": {
    ...
    "total_expenses": "إجمالي المصاريف العامة",
    "supplier_payments": "دفعات الموردين",
    "advances": "سلف العمال",
    "opening_balance": "الرصيد الافتتاحي / وارد الكاشير (+)",
    "actual_cash": "النقد الفعلي في الدرج (Actual)",
    "expected_cash": "المبلغ المتوقع (Expected):",
    "difference": "الفارق (Difference):",
    "notes": "ملاحظات (مبررات العجز أو الزيادة):",
    "save_btn": "حفظ وإغلاق اليوم"
  },
  "dashboard": {
    "title": "لوحة القيادة",
...
```json
{
  ...
  "currency": "DZD",
  "eod": {
    "title": "End of Day (EOD) Closure",
    "closure_date": "Closure Date:",
    "total_expenses": "Total General Expenses",
    "supplier_payments": "Supplier Payments",
    "advances": "Employee Advances",
    "opening_balance": "Opening Balance / Cashier Transfer (+)",
    "actual_cash": "Actual Cash in Drawer",
    "expected_cash": "Expected Cash:",
    "difference": "Difference:",
    "notes": "Notes (Justification for shortage/overage):",
    "save_btn": "Save & Close Day"
  },
