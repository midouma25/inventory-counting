...
// دالة لجلب ملخص المصاريف اليومية
function getDailySummary(date) {
  try {
    ...
    // تأكد من أسماء الأعمدة في جداولك، استعملت payment_date كمثال
...
const db = require('./database'); // تأكد من أن المسار يطابق اسم ملف قاعدة البيانات لديك
const path = require('path');
const { 
...
const EndOfDay = () => {
  ...
  const [summary, setSummary] = useState({ expenses: 0, supplierPayments: 0, advances: 0 });
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      const response = await window.api.getDailySummary(date);
      if (response.success) {
  ...
  // تم تصحيح summaryData إلى summary
  const totalOut = (summary.expenses || 0) + (summary.supplierPayments || 0) + (summary.advances || 0);
  const todaySales = actualAmount === '' || actualAmount === 0 
  ? 0 
  : (Number(actualAmount) + totalOut) - Number(openingBalance);

  const handleSaveEOD = async () => {
    // تم تحديث المتغيرات لتتوافق مع المنطق الجديد
    console.log("Saving...", { date, openingBalance, actualAmount, totalOut, todaySales, notes });
  };

  return (
    <div className="p-6 bg-gray-900 text-white min-h-screen rounded-lg">
      <h2 className="text-2xl font-bold mb-6 text-blue-400">{t('eod.title')}</h2>

      {/* اختيار التاريخ */}
      <div className="mb-6">
        <label className="block mb-2 text-gray-300">{t('eod.closure_date')}</label>
        <input 
          type="date" 
          value={date} 
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="p-4 bg-gray-800 rounded shadow border-t-4 border-red-500">
          <h3 className="text-gray-400">{t('eod.total_expenses')}</h3>
          <p className="text-xl font-bold">{summary.expenses} {t('currency')}</p>
        </div>
        <div className="p-4 bg-gray-800 rounded shadow border-t-4 border-red-500">
          <h3 className="text-gray-400">{t('eod.supplier_payments')}</h3>
          <p className="text-xl font-bold">{summary.supplierPayments} {t('currency')}</p>
        </div>
        <div className="p-4 bg-gray-800 rounded shadow border-t-4 border-red-500">
          <h3 className="text-gray-400">{t('eod.advances')}</h3>
          <p className="text-xl font-bold">{summary.advances} {t('currency')}</p>
        </div>
      </div>
      ...
      <div className="p-6 bg-gray-800 rounded-lg shadow-lg mb-6">
        <div className="grid grid-cols-2 gap-6 mb-4">
          <div>
            <label className="block mb-2 text-green-400 font-bold">{t('eod.opening_balance')}</label>
            <input 
              type="number" 
          ...
          <div>
            <label className="block mb-2 text-blue-400 font-bold">{t('eod.actual_cash')}</label>
            <input 
              type="number" 
              value={actualAmount} 
        ...
        {/* القسم الجديد: إجمالي المخصومات ومداخيل اليوم */}
        <div className="flex justify-between items-center border-t border-gray-600 pt-4 mt-4">
          <div>
            <h3 className="text-gray-400">{t('eod.total_deducted')}</h3>
            <p className="text-2xl font-bold text-red-500">{totalOut.toFixed(2)} {t('currency')}</p>
          </div>
          <div>
            <h3 className="text-gray-400">{t('eod.today_sales')}</h3>
            <p className={`text-3xl font-bold ${todaySales >= 0 ? 'text-green-500' : 'text-red-500'}`}>
              {todaySales > 0 ? '+' : ''}{todaySales.toFixed(2)} {t('currency')}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* ملاحظات وزر الحفظ */}
      <div className="mb-6">
        <label className="block mb-2 text-gray-300">{t('eod.notes')}</label>
        <textarea 
          value={notes} 
...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { path: '/agenda', name: t('sidebar.agenda'), icon: <Calendar size={20} /> },
    { path: '/end-of-day', name: t('sidebar.end_of_day'), icon: <Calendar size={20} /> },
  ];
...
```json
{
  ...
  "sidebar": {
    ...
    "hr": "الموارد البشرية",
    "expenses": "المصاريف",
    "payroll": "الرواتب والسلفيات",
    "end_of_day": "إغلاق اليوم (EOD)",
    "agenda": "الأجندة"
  },
"currency": "د.ج",
  "eod": {
    ...
    "advances": "سلف العمال",
    "opening_balance": "الرصيد الافتتاحي (في الصباح)",
    "actual_cash": "النقد الفعلي (الموجود في الدرج)",
    "total_deducted": "إجمالي الأموال المخصومة (مصاريف ودفعات)",
    "today_sales": "مداخيل اليوم (صافي المبيعات)",
    "notes": "ملاحظات",
    "save_btn": "حفظ إغلاق اليوم"
  },
  "dashboard": {
    "title": "لوحة القيادة",
...
"currency": "DZD",
  "eod": {
    ...
    "total_deducted": "Total Deductions (Expenses & Payments)",
    "today_sales": "Today's Sales (Net Income)",
    "notes": "Notes",
    "save_btn": "Save End of Day"
  },
