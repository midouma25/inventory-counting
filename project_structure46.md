...
OUTPUT_FILE = "project_structure46.md"
...
module.exports = {
  ...
  getInventoryTree, addInvFamily, deleteInvFamily, addInvType, deleteInvType, addInvItem, updateInvItem, deleteInvItem , updateInvFamily, updateInvType , getSystemNotifications, resetDatabase
...
export default function Payroll() {
  ...
  // 🌟 حالة نافذة الطباعة ونوع الطباعة (فردي أم كلي)
  const [isPrintModalOpen, setIsPrintModalOpen] = useState(false);
  const [printMode, setPrintMode] = useState('single'); // 'single' أو 'all'
      ...
      html += `
        ...
        <table style="border: none; margin-top: 50px;">
          <tr>
            ...
            <td style="border: none; border-top: 2px solid #000; width: 40%; font-weight: bold;">${t('print.employeeSignature', 'توقيع المستلم')}</td>
          </tr>
        </table>
      `;
    } 
  ...
  // 🌟 دالة طباعة كشف الراتب الحراري الفردي (اللون الأسود القوي فقط)
    ...
    doc.write(`
      ...
      <head>
        ...
        <style>
          ...
          .signatures { display: flex; justify-content: space-between; margin-top: 25px; font-size: 11px; font-weight: bold; color: #000; }
      ...
      <body>
        <div class="print-wrapper">
          <h2>${currentStoreName}</h2>
          <div class="subtitle">${t('payroll.payslip', 'كشف راتب موظف')}</div>
          
          <div style="text-align: center; font-size: 11px; font-weight: bold; margin-bottom: 10px; color: #000;">
          ...
          <table>
            ...
            <tr>
              <td style="font-weight: bold;">${t('payroll.deductions', 'الخصومات (السلف)')}:</td>
              <td style="text-align: ${isRTL ? 'left' : 'right'}; font-weight: bold;" dir="ltr">-${formatMoney(payrollResult.totalAdvances)} ${curr}</td>
            </tr>
          </table>
  ...
  // 🌟 دالة طباعة الأرشيف الكلي في ورقة حرارية طويلة (80mm)
  const handlePrintA7AllSalaries = () => {
    ...
    salaries.forEach((s) => {
      grandTotal += Number(s.net_salary);
      rowsHtml += `
        <div style="border-bottom: 1px dashed #000; padding: 6px 0; margin-bottom: 4px; color: #000;">
          <div style="display:flex; justify-content:space-between; font-weight:bold; font-size:14px; margin-bottom:4px;">
            <span>${s.employee_name || s.name}</span>
            <span dir="ltr">${formatMoney(s.net_salary)} ${curr}</span>
          </div>
          <div style="display:flex; justify-content:space-between; font-size:11px; font-weight:bold;">
            <span>${t('payroll.totalHours', 'الساعات')}: ${formatHours(s.total_hours)}</span>
            <span dir="ltr">${s.start_date.substring(5)} / ${s.end_date.substring(5)}</span>
          </div>
        </div>
      `;
    });
    ...
    doc.write(`
      ...
      <html lang="${i18n.language}" dir="${isRTL ? 'rtl' : 'ltr'}">
      <head>
        <title>All Salaries Thermal Print</title>
        <style>
          @page { margin: 0; }
          html, body { margin: 0; padding: 0; width: 72mm; background: #fff; color: #000; font-family: sans-serif; }
          .print-wrapper { width: 100%; padding: 2mm 5mm; box-sizing: border-box; }
          ...
          .subtitle { text-align: center; font-size: 13px; margin-bottom: 12px; border-bottom: 2px dashed #000; padding-bottom: 6px; font-weight: bold; color: #000; }
          .amount-box { display: flex; justify-content: space-between; align-items: center; border-top: 2px solid #000; border-bottom: 2px solid #000; padding: 8px 0; margin-top: 15px; color: #000; }
          .amount-box .box-title { font-size: 13px; font-weight: bold; padding: 0 5px; color: #000; }
          .amount-box .box-value { font-size: 16px; font-weight: 900; padding: 0 5px; color: #000; }
          .footer-brand { text-align: center; margin-top: 20px; font-size: 11px; font-weight: 900; border-top: 1px dashed #000; padding-top: 8px; color: #000; }
        </style>
      </head>
      <body>
        <div class="print-wrapper">
          <h2>${currentStoreName}</h2>
          <div class="subtitle">${t('payroll.comprehensiveReport', 'سجل الرواتب الكلي')}</div>
          ...
          <div class="amount-box">
            <span class="box-title">${t('payroll.totalEmployees', 'الإجمالي المدفوع')}:</span>
            <span class="box-value" dir="ltr">${formatMoney(grandTotal)} ${curr}</span>
          </div>

          <div style="text-align: center; margin-top: 25px; font-size: 12px; font-weight: bold; color: #000;">
            ${t('zreport.manager_sig', 'توقيع الإدارة / الختم')}<br><br>...........................
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 p-6 font-sans flex flex-col gap-6 text-start relative">
            ...
            {!payrollResult ? (
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95">
                <div className="bg-slate-800 p-6 flex justify-between items-center border-b border-slate-700">
                  ...
                  <button onClick={() => { setPrintMode('single'); setIsPrintModalOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 shadow-lg" title={t('inventory.printOptions', 'خيارات الطباعة')}>
      ...
      {activeTab === 'salaries' && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg">
          <div className="p-4 border-b border-slate-800 bg-slate-950/30 flex justify-between items-center">
            ...
            <button onClick={() => { setPrintMode('all'); setIsPrintModalOpen(true); }} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
              <Download size={18} /> {t('payroll.exportReportWord', 'استخراج سجل الرواتب')}
            </button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-start border-collapse" dir={i18n.dir()}>
      ...
      <Modal isOpen={isPrintModalOpen} onClose={() => setIsPrintModalOpen(false)} title={t('inventory.printOptions', 'خيارات الطباعة')}>
        <div className="p-6 flex flex-col gap-4 text-start" dir={isRTL ? 'rtl' : 'ltr'}>
          <p className="text-slate-400 mb-4 text-center">{t('inventory.printDesc', 'اختر مقاس الورق المناسب للطباعة.')}</p>
          
          <button 
            onClick={() => {
              printMode === 'single' ? handleExportSingleToWord() : exportSalariesToWord(salaries); 
              setIsPrintModalOpen(false); 
            }} 
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="text-start">
                ...
                <div className="text-xs font-normal opacity-80 mt-1">{t('payroll.printA4Desc', 'مستند وورد شامل يحتوي على التفاصيل لحفظه في أرشيف الشركة.')}</div>
              </div>
            </div>
            <FileText size={20} />
          </button>
          ...
          <button 
            onClick={() => {
              printMode === 'single' ? handlePrintA7Payslip() : handlePrintA7AllSalaries();
            }} 
...
"payroll": {
    ...
    "calcSalary": "حساب الراتب",
    "emptyStateDesc": "أدخل بيانات الموظف واضغط على حساب الراتب لظهور النتيجة",
    "period": "الفترة",
    "totalHours": "إجمالي الساعات",
    "grossSalary": "الراتب الإجمالي",
    "deductions": "الخصومات / السلف",
    ...
    "caisseAdmin": "صندوق المدير (الرئيسي)",
    "caisseCashier": "صندوق الكاشير: {{name}}",
    "note": "ملاحظة",
    "advanceNotePlaceholder": "سبب السلفة...",
    "saveAdvance": "حفظ السلفة",
    "statusPending": "غير مسددة",
    "statusPaid": "تم الخصم",
    "editAdvance": "تعديل السلفة",
    "newAdvance": "تسجيل سلفة موظف",
    "exportReportWord": "استخراج سجل الرواتب",
    "printA4": "تحميل كشف مفصل Word (A4)",
    "printA4Desc": "مستند وورد شامل يحتوي على التفاصيل لحفظه في أرشيف الشركة.",
    "printA7": "وصل طباعة حرارية (80mm)",
    "printA7Desc": "وصل ورقي سريع ومقروء يُطبع فوراً من طابعة الكاشير.",
    "payslip": "كشف راتب موظف",
    "comprehensiveReport": "سجل الرواتب الكلي",
    "totalEmployees": "الإجمالي المدفوع",
    "confirmPayTitle": "تأكيد دفع الراتب",
    "confirmPayMessage": "الراتب الصافي بالسالب. سيتم ترحيل الديون المتبقية كسلفة جديدة للشهر القادم. هل تود الاستمرار؟",
    "confirmPayStandard": "سيتم صرف مبلغ {{amount}} من الصندوق. هل تود تأكيد الدفع؟",
    "confirmPayBtn": "نعم، تأكيد الدفع",
    "cancelAdvanceTitle": "إلغاء السلفة",
    "cancelAdvanceMessage": "هل أنت متأكد من إلغاء وحذف هذه السلفة؟ سيتم إرجاع المبلغ للصندوق الذي خُصمت منه.",
    "cancelAdvanceBtn": "نعم، حذف السلفة",
    "errors": {
      "overlap": "لا يمكن الحساب! لقد تم دفع راتب مسبقاً ({{start}} إلى {{end}})"
    },
  ...
  "database": {
    ...
    "resetTitle": "بدء بيانات جديدة",
    "resetDesc": "تصفير كل الإحصائيات والجداول للبدء من جديد (يصلح بعد انتهاء فترة التدريب).",
    "resetBtn": "تصفير النظام",
    "resetWarningTitle": "هل أنت متأكد من تصفير النظام؟",
    "resetWarningMsg": "سيتم مسح جميع المبيعات، الموردين، الجرد، والرواتب بالكامل ولن تتمكن من استرجاعها. سيتم إبقاء حساب الأدمن الافتراضي فقط.",
    "resetWithBackup": "حفظ نسخة احتياطية ثم التصفير (موصى به)",
    "resetWithoutBackup": "تصفير النظام مباشرة (مسح نهائي)",
    "messages": {
      ...
      "resetSuccess": "تم تصفير النظام بنجاح! سيتم إعادة توجيهك لتسجيل الدخول."
    }
  },
  "audit": {
...
"inventory": {
    ...
    "price": "السعر",
    "quickStock": "جرد سريع",
    "wordTitle": "ورقة جرد المخزون",
    "description": "أنشئ قوائم الجرد واطبعها.",
    "addFamily": "عائلة",
    "addType": "نوع",
    "addName": "إسم",
    "addProduct": "منتج",
    "addFamilyTitle": "إضافة عائلة",
    "addTypeTitle": "إضافة نوع",
    "addProductTitle": "إضافة منتجات للمخزون",
    "addNameTitle": "إضافة أسماء للنوع",
    "editFamilyTitle": "تعديل اسم العائلة",
    "editTypeTitle": "تعديل اسم النوع",
    "editProductTitle": "تعديل تفاصيل المنتج",
    "namesDictHint": "أدخل أسماء المنتجات (كل منتج في سطر جديد)",
    "namesPlaceholder": "الحارة\nإيزيس\nعصير رامي\n...",
    "selectNamesAndPrices": "اختر المنتجات وحدد السعر والعلبة لكل واحد:",
    "emptyDict": "القائمة فارغة، قم بإضافة أسماء من زر (+ إسم)",
    "selectOneName": "الرجاء تحديد منتج واحد على الأقل",
    "selectTypeFirst": "يرجى اختيار العائلة والنوع أولاً",
    "allFamilies": "جميع العائلات",
    "allTypes": "جميع الأنواع",
    "cancelDigitalMode": "إلغاء الجرد الرقمي",
    "enterDigitalMode": "إدخال جرد رقمي",
    "printSheet": "طباعة ورقة جرد",
    "category": "التصنيف",
    "systemQty": "الكمية (في النظام)",
    "actualQty": "الكمية (الواقعية)",
    "family": "العائلة",
    "selectFamily": "اختر العائلة",
    "type": "نوع المنتج",
    "selectType": "اختر النوع",
    "printOptions": "خيارات الطباعة",
    "printDesc": "اختر مقاس الورق المناسب لطباعة القائمة المحددة أو الحالية.",
    "printA4": "تحميل ملف Word (A4)",
    "printA4Desc": "مستند وورد مقسوم على عمودين لتوفير المساحة",
    "printA7": "ورقة جرد حرارية (80mm)",
    "printA7Desc": "جرد سريع للمنطقة المحددة (طابعة الكاشير)",
    "deleteWarning": "هل أنت متأكد من الحذف؟ سيتم حذف جميع المتفرعات التابعة.",
    "sortByCategory": "ترتيب حسب التصنيف",
    "clearQty": "تفريغ",
    "qtyCleared": "تم تصفير الكمية وتفريغها",
    "seedBtn": "تغذية تلقائية بماركات السوق",
    "seedSuccess": "تم شحن قاعدة البيانات بآلاف الماركات بنجاح! 🚀",
    "marketDbBtn": "قاعدة السوق",
    "marketDbTitle": "استيراد قاعدة بيانات السوق",
    "marketDbDesc": "اختر العائلة المناسبة لكل نوع من الأنواع الجاهزة لكي يتم استيراد منتجاتها تلقائياً. (الأنواع التي تتجاهلها لن يتم استيرادها)",
    "productsList": "المنتجات",
    "assignToFamily": "تعيين لعائلة",
    "selectedTypes": "الأنواع المحددة:",
    "selectOneFamilyAtLeast": "الرجاء تعيين عائلة واحدة على الأقل"
  },
"badge": {
      "printBtn": "بطاقة موظف",
    ...
    "agendaTitle": "مهمة في الأجندة",
    "agendaDesc": "لديك مهمة مستحقة: {{title}}",
    "debtTitle": "تنبيه ديون الموردين",
    "debtDesc": "المورد {{name}} يدين لك بمبلغ {{debt}} د.ج",
    "thursdayTitle": "تنبيه يوم الرواتب ⚠️",
    "thursdayDesc": "اليوم الخميس هو يوم دفع الرواتب. يرجى إغلاق الورديات وإجراء الصندوق وتصفية حسابات العمال لتفادي الأخطاء."
  }
}
```
...
"payroll": {
    "title": "Staff Payroll",
    "subtitle": "Manage and calculate salaries and deductions",
    "tabs": {
      "calculator": "Salary Calculator",
      "advances": "Advances Log",
      "salaries": "Salaries Archive"
    },
    "selectEmployee": "Select Employee",
    "hourlyRate": "Hourly Rate",
    "calcSalary": "Calculate Salary",
    "emptyStateDesc": "Enter employee details and click 'Calculate Salary' to view results",
    "period": "Period",
    ...
    "caisseAdmin": "Admin Fund (Main)",
    "caisseCashier": "Cashier Fund: {{name}}",
    "note": "Note",
    "advanceNotePlaceholder": "Reason for advance...",
    "saveAdvance": "Save Advance",
    "statusPending": "Unpaid",
    "statusPaid": "Deducted",
    "editAdvance": "Edit Advance",
    "newAdvance": "Record Employee Advance",
    "exportReportWord": "Export Report",
    "printA4": "Download Detailed Word (A4)",
    "printA4Desc": "Comprehensive Word document with details for company archives.",
    "printA7": "Thermal Print Ticket (80mm)",
    "printA7Desc": "Fast and readable paper ticket printed instantly from the cashier printer.",
    "payslip": "Employee Payslip",
    "comprehensiveReport": "Total Salaries Report",
    "totalEmployees": "Total Paid",
    "confirmPayTitle": "Confirm Payment",
    "confirmPayMessage": "Net salary is negative. Remaining debts will be rolled over as a new advance for next month. Continue?",
    "confirmPayStandard": "An amount of {{amount}} will be deducted from the fund. Do you confirm payment?",
    "confirmPayBtn": "Yes, Pay",
    "cancelAdvanceTitle": "Cancel Advance",
    "cancelAdvanceMessage": "Are you sure you want to cancel this advance? The amount will be refunded to the fund.",
    "cancelAdvanceBtn": "Yes, Delete",
    "errors": {
      "overlap": "Cannot calculate! A salary was already paid ({{start}} to {{end}})"
    },
    "rolloverNote": "Rollover debt ({{start}} to {{end}})",
  ...
  "database": {
    ...
    "resetTitle": "Start Fresh Data",
    "resetDesc": "Reset all statistics and tables to start over (ideal after finishing the training period).",
    "resetBtn": "Factory Reset",
    "resetWarningTitle": "Are you sure you want to reset?",
    "resetWarningMsg": "All sales, suppliers, inventory, and payrolls will be permanently deleted. Only the default admin account will be kept.",
    "resetWithBackup": "Backup then Reset (Recommended)",
    "resetWithoutBackup": "Reset directly (Permanent Deletion)",
    "messages": {
      ...
      "resetSuccess": "System reset successfully! Redirecting to login."
    }
  },
  "audit": {
...
"inventory": {
    ...
    "wordTitle": "Inventory Sheet",
    "description": "Build your database and print inventory sheets for stock tracking.",
    "addFamily": "Family",
    "addType": "Type",
    "addName": "Name",
    "addProduct": "Product",
    ...
    "addNameTitle": "Add names to dictionary",
    "editFamilyTitle": "Edit Family",
    "editTypeTitle": "Edit Type",
    "editProductTitle": "Edit Product",
    "namesDictHint": "Enter product names (one per line)",
    "namesPlaceholder": "Item 1\nItem 2\n...",
    "selectNamesAndPrices": "Select products and set price & box for each:",
    "emptyDict": "List is empty, add names via (+ Name) button",
    "selectOneName": "Please select at least one product",
    "selectTypeFirst": "Please select family and type first",
    "allFamilies": "All Families",
    "allTypes": "All Types",
    "cancelDigitalMode": "Cancel Digital Mode",
    "enterDigitalMode": "Digital Stocktaking",
    ...
    "printDesc": "Choose paper format for printing.",
    "printA4": "Download Word (A4)",
    "printA4Desc": "Document split into two columns",
    "printA7": "Thermal Print (80mm)",
    "printA7Desc": "Quick print for selected area",
    "deleteWarning": "Are you sure? All dependent items will be deleted.",
    "sortByCategory": "Sort by Category",
    "clearQty": "Clear",
    "qtyCleared": "Quantity cleared successfully",
    "seedBtn": "Load Local Brands",
    "seedSuccess": "Database loaded successfully! 🚀",
    "marketDbBtn": "Market DB",
    "marketDbTitle": "Import Market Database",
    "marketDbDesc": "Choose the appropriate family for each ready-made type to automatically import its products. (Ignored types will not be imported)",
    "productsList": "Products",
    "assignToFamily": "Assign to Family",
    "selectedTypes": "Selected Types:",
    "selectOneFamilyAtLeast": "Please assign at least one family"
  },
"badge": {
    ...
    "agendaTitle": "Agenda Task",
    "agendaDesc": "You have a due task: {{title}}",
    "debtTitle": "Supplier Debts",
    "debtDesc": "Supplier {{name}} has a debt of {{debt}} DA",
    "thursdayTitle": "Payroll Day Alert ⚠️",
    "thursdayDesc": "Today is Thursday (Payroll day). Please don't forget to close shifts and settle staff accounts."
  }
  }
...
"payroll": {
    ...
    "subtitle": "Gérer et calculer les salaires et les déductions",
    "tabs": {
      "calculator": "Calculatrice de Salaire",
      "advances": "Registre des Avances",
      "salaries": "Archives des Salaires"
    },
    "selectEmployee": "Sélectionner un employé",
    "hourlyRate": "Taux horaire",
    "calcSalary": "Calculer le salaire",
    "emptyStateDesc": "Entrez les détails de l'employé et cliquez sur 'Calculer le salaire'",
    "period": "Période",
    "totalHours": "Total des Heures",
    "grossSalary": "Salaire Brut",
    "deductions": "Déductions / Avances",
    "netSalary": "Net à Payer",
    "payAndSave": "Payer et Enregistrer",
    "addAdvance": "Ajouter Avance",
    "date": "Date",
    "amount": "Montant",
    "fundSource": "Source des fonds",
    "caisseAdmin": "Caisse Admin (Principale)",
    "caisseCashier": "Caisse Caissier: {{name}}",
    "note": "Note",
    "advanceNotePlaceholder": "Raison de l'avance...",
    "saveAdvance": "Enregistrer Avance",
    "statusPending": "Non Payé",
    "statusPaid": "Déduit",
    "editAdvance": "Modifier Avance",
    "newAdvance": "Nouvelle Avance",
    "exportReportWord": "Exporter le rapport",
    "printA4": "Télécharger Word Détaillé (A4)",
    "printA4Desc": "Document Word complet avec les détails pour les archives de l'entreprise.",
    "printA7": "Ticket Thermique (80mm)",
    "printA7Desc": "Ticket rapide et lisible imprimé instantanément sur l'imprimante de caisse.",
    "payslip": "Fiche de Paie",
    "comprehensiveReport": "Rapport Global des Salaires",
    "totalEmployees": "Total Payé",
    "confirmPayTitle": "Confirmer le paiement",
    "confirmPayMessage": "Le salaire net est négatif. Les dettes seront reportées comme une nouvelle avance pour le mois prochain. Continuer ?",
    "confirmPayStandard": "Un montant de {{amount}} sera déduit de la caisse. Confirmez-vous le paiement ?",
    "confirmPayBtn": "Oui, Payer",
    "cancelAdvanceTitle": "Annuler l'avance",
    "cancelAdvanceMessage": "Voulez-vous vraiment annuler cette avance ? Le montant sera remboursé à la caisse.",
    "cancelAdvanceBtn": "Oui, Supprimer",
    "errors": {
      "overlap": "Impossible ! Un salaire a déjà été payé ({{start}} à {{end}})"
    },
    "rolloverNote": "Report des dettes ({{start}} à {{end}})",
    "expenseNote": "Salaire : {{name}} ({{start}} à {{end}})"
  },
    "modal": {
  ...
  "database": {
    ...
    "resetTitle": "Nouvelles données",
    "resetDesc": "Réinitialiser toutes les statistiques et les tables pour recommencer à zéro (idéal après la formation).",
    "resetBtn": "Réinitialiser",
    "resetWarningTitle": "Êtes-vous sûr de vouloir réinitialiser ?",
    "resetWarningMsg": "Toutes les ventes, fournisseurs, inventaires et salaires seront effacés définitivement. Seul le compte administrateur sera conservé.",
    "resetWithBackup": "Sauvegarder puis réinitialiser (Recommandé)",
    "resetWithoutBackup": "Réinitialiser directement (Suppression définitive)",
    "messages": {
      ...
      "resetSuccess": "Système réinitialisé avec succès ! Vous allez être redirigé vers la connexion."
    }
  },
  "audit": {
...
"inventory": {
    ...
    "product": "Produit",
    "price": "Prix",
    "quickStock": "Inventaire Rapide",
    "wordTitle": "Fiche d'inventaire",
    "description": "Créez la base de données puis imprimez les fiches d'inventaire.",
    "addFamily": "Famille",
    "addType": "Type",
    "addName": "Nom",
    "addProduct": "Produit",
    "addFamilyTitle": "Ajouter Famille",
    "addTypeTitle": "Ajouter Type",
    "addProductTitle": "Ajouter des Produits au Stock",
    "addNameTitle": "Ajouter des noms au dictionnaire",
    "editFamilyTitle": "Modifier la famille",
    "editTypeTitle": "Modifier le type",
    "editProductTitle": "Modifier le produit",
    "namesDictHint": "Entrez les noms des produits (un par ligne)",
    "namesPlaceholder": "Tomate Hra\nJus Ramy\n...",
    "selectNamesAndPrices": "Sélectionnez les produits et définissez le prix et la boîte :",
    "emptyDict": "La liste est vide, cliquez sur + Nom pour en ajouter",
    "selectOneName": "Veuillez sélectionner au moins un produit",
    "selectTypeFirst": "Veuillez d'abord sélectionner la famille et le type",
    "allFamilies": "Toutes les familles",
    "allTypes": "Tous les types",
    "cancelDigitalMode": "Annuler la saisie",
    "enterDigitalMode": "Saisie Numérique",
    "printSheet": "Imprimer Fiche",
    "category": "Catégorie",
    "systemQty": "Qté Système",
    "actualQty": "Qté Réelle",
    "family": "Famille",
    "selectFamily": "Sélectionnez la famille",
    "type": "Type de Produit",
    "selectType": "Sélectionnez le type",
    "printOptions": "Options d'impression",
    "printDesc": "Choisissez le format de papier approprié.",
    "printA4": "Télécharger Word (A4)",
    "printA4Desc": "Document divisé en deux colonnes pour économiser l'espace",
    "printA7": "Ticket d'inventaire (80mm)",
    "printA7Desc": "Impression thermique rapide",
    "deleteWarning": "Voulez-vous vraiment supprimer ? Tous les éléments dépendants seront supprimés.",
    "sortByCategory": "Trier par Catégorie",
    "clearQty": "Effacer",
    "qtyCleared": "Quantité effacée avec succès",
    "seedBtn": "Charger Marques (Algérie)",
    "seedSuccess": "Base de données chargée avec succès! 🚀",
    "marketDbBtn": "Base du Marché",
    "marketDbTitle": "Importer la base de données du marché",
    "marketDbDesc": "Choisissez la famille appropriée pour chaque type prêt à l'emploi afin d'importer automatiquement ses produits. (Les types ignorés ne seront pas importés)",
    "productsList": "Produits",
    "assignToFamily": "Assigner à une famille",
    "selectedTypes": "Types sélectionnés :",
    "selectOneFamilyAtLeast": "Veuillez assigner au moins une famille"
  },
"badge": {
    ...
    "agendaTitle": "Tâche d'Agenda",
    "agendaDesc": "Vous avez une tâche à faire : {{title}}",
    "debtTitle": "Dettes Fournisseurs",
    "debtDesc": "Le fournisseur {{name}} a une dette de {{debt}} DA",
    "thursdayTitle": "Jour de Paie ⚠️",
    "thursdayDesc": "C'est jeudi, jour de paie. N'oubliez pas de clôturer les caisses et de régler les comptes du personnel."
  }
}
```
